...
- [[Sliding Window]] / [[Two Pointers]] para contagem em tempo $O(1)$ real de elementos distintos em janelas dinâmicas onde o $O(\log N)$ do `std::map` causa TLE por constante oculta.

- Sub-rotina de busca de estados transitórios em buscas de grafos de alta dimensionalidade (como BFS em problemas de _8-Puzzle_ ou matrizes compactadas em inteiros de 64 bits via bitmask).

- [[Memoização]] em Programação Dinâmica onde o domínio do estado é esparso e linear em tempo.